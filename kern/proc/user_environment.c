...
void env_free(struct Env *e)
{
   ...
   // 1- remove All pages in the page working set , Working set itself
	struct WorkingSetElement *wse;
	LIST_FOREACH(wse, &(e->page_WS_list))
	{
	...
	LIST_FOREACH(wse, &(e->page_WS_list))
	{
		uint32* ptr_page_table;
		uint32 * ptr;
		get_page_table(e->env_page_directory, wse->virtual_address, &ptr_page_table);
	...
	for(uint32 va=(uint32)USER_HEAP_START;va<USER_HEAP_MAX;va+=PAGE_SIZE)
	{
		uint32 *ptr_page_table;
...
void env_exit(void)
{
	...
	//int freePages1 = calculate_free_frames();
	//cprintf("freePages %d\n",freePages1);
...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
	...
	if (stack_base==NULL)
	{
		panic("Failed to allocate memory for user kernel stack");
	}
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
	...
	e->U_heap_start = (uint32 *)daStart;
	e->U_heap_break = (uint32 *)daStart;
	e->U_heap_limit = (uint32 *)daLimit;
