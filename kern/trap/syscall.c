...
void sys_allocate_user_mem(uint32 virtual_address, uint32 size)
{
	...
	if((uint32*)virtual_address!=NULL){
		if((uint32*)virtual_address<=(uint32*)USER_HEAP_MAX&&(uint32*)virtual_address>=(uint32*)USER_HEAP_START){
...
void sys_move_user_mem(uint32 src_virtual_address, uint32 dst_virtual_address, uint32 size)
{
	//TODO: [PROJECT'24.MS1 - #03] [2] SYSTEM CALLS - Params Validation
		...
		if((uint32*)src_virtual_address!=NULL){
			if((uint32*)src_virtual_address>=(uint32*)USER_HEAP_START&&(uint32*)src_virtual_address<=(uint32*)USER_HEAP_MAX){
...
void sys_enqueue(struct semaphore* sem){

    acquire_spinlock(&ProcessQueues.qlock);
    struct Env *curr_process = get_cpu_proc();
    enqueue (&(sem->semdata->queue), curr_process);
    sem->semdata->lock=0;
    curr_process->env_status = ENV_BLOCKED;
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_allocate_user_mem:
		...
		break;
	case SYS_enqueue:
		sys_enqueue((struct semaphore *)a1);
		return 0;
		break;
