...
int process_command(int number_of_arguments, char** arguments) {
	...
	LIST_INIT(&foundCommands);
	int found_exact_match = 0;
	int found_partial_match = 0;
	for (int i = 0; i < NUM_OF_COMMANDS; i++) {
		if (strcmp(arguments[0], commands[i].name) == 0) {
			...
			} else if (number_of_arguments == commands[i].num_of_args + 1) {
				return i;
			} else {
				LIST_INSERT_HEAD(&foundCommands, &commands[i]);
				return CMD_INV_NUM_ARGS;
	...
	for (int i = 0; i < NUM_OF_COMMANDS; i++) {
		...
		for (int j = 0; j < length; j++) {
			if (*strfind(commands[i].name, arguments[0][j]) == arguments[0][j]) {
				partial_match_found = 1;
			} else {
				partial_match_found = 0;
				break;
			}
		}
