...
void sched_init_PRIRR(uint8 numOfPriorities, uint8 quantum, uint32 starvThresh)
{
    ...
    {
        init_queue(&ProcessQueues.env_ready_queues[i]); // initializes an empty queue
    }
    //quantum_counter = 0;
...
struct Env* fos_scheduler_PRIRR()
{
    ...
    for(int i = 0 ; i < (int)num_of_ready_queues ; i++)
	{
...
void clock_interrupt_handler(struct Trapframe* tf)
{
	...
	for(int i = 0 ; i < num_of_ready_queues; i++)
	{
		...
		{
			if (ticks*quantums[0] >= st)
			{
				acquire_spinlock(&ProcessQueues.qlock);
				sched_remove_ready(env);
				if(env->priority!=0)
				{
					env_set_priority(env->env_id,env->priority-1);
				}
